...
public class DubboMetaDaoImpl extends BaseDao implements IDubboMetaDao {
    ...
    public long add(DubboMeta dubboMeta) {
        String sql = "insert into apigw_gportal_dubbo_meta (application_name, protocol_version, interface_name, dubbo_group, dubbo_version, method, dubbo_params, dubbo_returns, create_time, gw_id)"
                + "values(:applicationName, :protocolVersion, :interfaceName, :dubboGroup, :dubboVersion, :method, :dubboParams, :dubboReturns, :createTime, :gwId)";
    ...
    @Override
    public long update(DubboMeta dubboMeta) {
        String sql = "update apigw_gportal_dubbo_meta set application_name=:applicationName, protocol_version=:protocolVersion, interface_name=:interfaceName, dubbo_group=:dubboGroup, dubbo_version=:dubboVersion, method=:method, dubbo_params=:dubboParams, dubbo_returns=:dubboReturns, create_time=:createTime, gw_id=:gwId where id=:id";
        SqlParameterSource ps = new BeanPropertySqlParameterSource(dubboMeta);
    ...
    public int delete(DubboMeta dubboMeta) {
        String sql = "delete from apigw_gportal_dubbo_meta where id=:id";
        SqlParameterSource ps = new BeanPropertySqlParameterSource(dubboMeta);
    ...
    public List<DubboMeta> getRecordsByField(Map<String, Object> params) {
        String head = "select * from apigw_gportal_dubbo_meta where ";
    ...
    @Override
    public void batchDeleteByCondition(long gwId, String interfaceName, String group, String version) {
        String sql = "delete from apigw_gportal_dubbo_meta where interface_name=:interfaceName and dubbo_group=:dubboGroup and dubbo_version=:dubboVersion and gw_id=:gwId";
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("interfaceName", interfaceName);
        mapSqlParameterSource.addValue("gwId", gwId);
        mapSqlParameterSource.addValue("dubboGroup", group);
        mapSqlParameterSource.addValue("dubboVersion", version);
        logger.info("deleteDubboMetaInfo gwId = {} interfaceName = {} ,group = {} ,version = {}", gwId, interfaceName, group, version);
        namedParameterJdbcTemplate.update(sql, mapSqlParameterSource);
    }

    @Override
    public int getCountByFields(Map<String, Object> params) {
        String head = "select count(*) from apigw_gportal_dubbo_meta where ";
        String sql = getQueryCondition(head, params);

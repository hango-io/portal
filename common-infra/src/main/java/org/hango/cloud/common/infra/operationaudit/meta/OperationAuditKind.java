...
public class OperationAuditKind {
    ...
    static {
        ...
        /*********************************************** Gateway End *****************************************************/
        /*********************************************** Service Start *****************************************************/
        kindMap.put("CreateService", OperationAuditRule.get().eventName("CreateService").description("创建服务").resourceType(AuditConst.AUDIT_RESOURCE_TYPE_SERVICE).jsonPathForRSNReq("$.ServiceName").jsonPathForRSIResp(DEFAULT_RETURN));
        kindMap.put("UpdateService", OperationAuditRule.get().eventName("UpdateService").description("编辑服务基本信息").resourceType(AuditConst.AUDIT_RESOURCE_TYPE_SERVICE).jsonPathForRSIReq("$.ServiceId").jsonPathForRSIReq("$.ServiceName"));
        kindMap.put("DeleteService", OperationAuditRule.get().eventName("DeleteService").description("删除服务").resourceType(AuditConst.AUDIT_RESOURCE_TYPE_SERVICE).readRSIFromQuery(SERVICE_ID).jsonPathForRSNResp(DEFAULT_RETURN));
        /*********************************************** Service End *****************************************************/

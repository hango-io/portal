...
public class PluginInfoController extends AbstractController {
    ...
    public String updatePluginConfiguration(@RequestBody PluginBindingDto pluginBindingDto) {
        ...
        PluginBindingDto pluginInfo = pluginInfoService.get(pluginBindingDto.getId());
        pluginInfo.setTemplateId(pluginBindingDto.getTemplateId());
        pluginInfo.setPluginConfiguration(pluginBindingDto.getPluginConfiguration());
        pluginInfoConvertService.fillPluginInfo(pluginInfo);
    ...
    public String getBindingPlugins(@Validated PluginBindingQueryDto queryDto) {
        logger.info("查询插件绑定列表, queryDto:{}", JSONObject.toJSONString(queryDto));
        ErrorCode checkResult = pluginInfoService.checkDescribeBindingPlugins(queryDto.getBindingObjectId(), queryDto.getBindingObjectType());
        if (!CommonErrorCode.SUCCESS.equals(checkResult)) {
            return apiReturn(checkResult);
        }
        Map<String, Object> result = Maps.newHashMap();

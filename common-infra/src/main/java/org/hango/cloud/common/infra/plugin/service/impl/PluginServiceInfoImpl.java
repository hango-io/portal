...
import org.hango.cloud.common.infra.plugin.service.PluginCacheService;
...
public class PluginServiceInfoImpl implements IPluginInfoService {
    ...
    public ErrorCode checkCreateParam(PluginBindingDto pluginBindingDto) {
        ...
        if (PluginBindingInfo.BINDING_OBJECT_TYPE_ROUTE_RULE.equals(bindingPluginDto.getBindingObjectType())) {
            RouteRuleProxyDto routeRuleProxyDto = routeRuleProxyService.getRouteRuleProxy(bindingPluginDto.getVirtualGwId(), bindingPluginDto.getBindingObjectId());
            if (null == routeRuleProxyDto) {
            ...
            DomainInfoPO domainInfoPO = domainInfoMapper.selectById(bindingPluginDto.getBindingObjectId());
            ...
            ServiceProxyDto serviceProxyDto = serviceProxyService.getServiceProxyByServiceIdAndGwId(bindingPluginDto.getVirtualGwId(), bindingPluginDto.getBindingObjectId());
    ...
    private void fillDtoFiled(List<PluginBindingDto> pluginBindingDtoList) {
        ...
        Set<Long> gwIdSet = pluginBindingDtoList.stream().map(PluginBindingDto::getVirtualGwId).collect(Collectors.toSet());
        ...
        Map<Long, VirtualGatewayDto> gatewayInfoMap = virtualGatewayInfoService.getGatewayInfoList(Lists.newArrayList(gwIdSet)).stream().collect(Collectors.toMap(VirtualGatewayDto::getId, item -> item));
        ...
        Map<Long, RouteRuleDto> routeRuleInfoMap = routeRuleInfoService.getRouteRuleList(Lists.newArrayList(routeRuleIdSet)).stream().collect(Collectors.toMap(RouteRuleDto::getId, item -> item));
        ...
        Map<Long, DomainInfoPO> domainInfoMap = new HashMap<>();
        if (!CollectionUtils.isEmpty(domainIdSet)){
            domainInfoMap = domainInfoMapper.selectBatchIds(domainIdSet).stream().collect(Collectors.toMap(DomainInfoPO::getId, item -> item));
        }
        for (PluginBindingDto item : pluginBindingDtoList) {
            fillDtoFiiled(gatewayInfoMap, serviceInfoMap, routeRuleInfoMap, templateInfoMap, domainInfoMap, item);

        }
    }

    private static void fillDtoFiiled(Map<Long, VirtualGatewayDto> gatewayInfoMap, Map<Long, ServiceDto> serviceInfoMap, Map<Long, RouteRuleDto> routeRuleInfoMap, Map<Long, PluginTemplateDto> templateInfoMap, Map<Long, DomainInfoPO> domainMap, PluginBindingDto item) {
        ...
        if (PluginBindingInfo.BINDING_OBJECT_TYPE_ROUTE_RULE.equals(item.getBindingObjectType())) {
            RouteRuleDto routeRuleInfo = routeRuleInfoMap.get(Long.valueOf(item.getBindingObjectId()));
            ...
            DomainInfoPO domainInfoPO = domainMap.get(Long.valueOf(item.getBindingObjectId()));
    ...
    public PluginBindingDto toView(PluginBindingInfo pluginBindingInfo) {
         ...
         */
        pluginBindingDto.setPluginName(pluginCacheService.getPluginNameFromCache(pluginBindingInfo.getPluginType()));

...
import static org.hango.cloud.common.infra.base.meta.LockConstant.REDIS_LOCK_LOG_PRE;
...
public class MethodReentrantLockAspect {
    ...
    public Object around(ProceedingJoinPoint pjp, MethodReentrantLock methodReentrantLock) throws Throwable {
        ...
        try {
            ...
            log.warn("{} Get Redis lock failed, error message: {}", REDIS_LOCK_LOG_PRE, e.getMessage());
        }
        ...
        long timeout;
        if (methodReentrantLock.tryLockTimeout() > 0){
            timeout = methodReentrantLock.tryLockTimeout();
        }else {
        ...
        try {
            ...
            log.warn("{} Redis lock error, error message: {}", REDIS_LOCK_LOG_PRE, e.getMessage());
        }
        if (!isLocked && methodReentrantLock.interrupt()){
            // 获取锁超时并且设置中断，则抛出异常不再执行此次提交
            throw new DistributedLockTimeOutException("try lock time out.");
        }
        ...
        try {
            ...
            if (isLocked) {
                try {
                    ...
                    log.error("methodReentrantLock unlock error");
                }
            }
        }
    }
    ...
    private String getValue(Object o) {
        ...
        try {
            ...
            log.error("{} Object transform to String JSON fail, err msg: {}", REDIS_LOCK_LOG_PRE, e.getMessage());
        }
        if (json == null) {
            json = "{}";
        }

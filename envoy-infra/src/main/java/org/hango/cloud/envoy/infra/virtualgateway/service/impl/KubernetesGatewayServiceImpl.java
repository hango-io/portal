...
public class KubernetesGatewayServiceImpl implements IKubernetesGatewayService {
    ...
    public List<KubernetesGatewayDTO> getKubernetesGatewayList(Long virtualGatewayId) {
        List<DomainInfoDTO> domainInfoDTOS = domainInfoService.getRelevanceOnlyDomainInfos(virtualGatewayId);
    ...
    private ErrorCode doRefreshK8sGateway(GatewayDto gatewayDto){
        ...
        List<VirtualGatewayDto> dbResources = virtualGatewayInfoService.getGatewayList(gatewayDto.getId(), KUBERNETES_GATEWAY);
        //数据处理，集合需要删除、创建或更新的数据
    ...
    private ErrorCode portDuplicateCheck(List<KubernetesGatewayInfo> kubernetesGatewayInfos, long gwId){
        ...
        List<VirtualGatewayDto> dbResources = virtualGatewayInfoService.getGatewayInfoByGwId(gwId).stream()
    ...
    private void handleDeleteResource(List<VirtualGatewayDto> virtualGatewayDtos){
        for (VirtualGatewayDto virtualGatewayDto : virtualGatewayDtos) {
            ...
            List<DomainInfoDTO> domainInfoDTOS = domainInfoService.getRelevanceOnlyDomainInfos(vgId);
            for (DomainInfoDTO domainInfoDTO : domainInfoDTOS) {
                //删除插件
                List<PluginBindingDto> pluginBindingList = pluginInfoService.getPluginBindingList(vgId, String.valueOf(domainInfoDTO.getId()), PluginBindingInfo.BINDING_OBJECT_TYPE_HOST);
    ...
    private void handleCreateResource(List<KubernetesGatewayInfo> kubernetesGatewayInfos, GatewayDto gatewayDto){
        for (KubernetesGatewayInfo kubernetesGatewayInfo : kubernetesGatewayInfos) {
            ...
            kubernetesGatewayInfo.setVirtualGatewayId(virtualGatewayDto.getId());
            //创建域名
    ...
    private void handleUpdateResource(List<KubernetesGatewayInfo> kubernetesGatewayInfos, GatewayDto gatewayDto){
        for (KubernetesGatewayInfo kubernetesGatewayInfo : kubernetesGatewayInfos) {
            ...
            pluginManagerService.publishPluginManager(virtualGatewayDto);
            //更新域名
            handleRefreshDomain(kubernetesGatewayInfo, gatewayDto);
        }
    }

    private void handleRefreshDomain(KubernetesGatewayInfo kubernetesGatewayInfo, GatewayDto gatewayDto){
        List<String> targetHosts = kubernetesGatewayInfo.getRouteHosts();
        List<DomainInfoDTO> dbDomains = domainInfoService.getRelevanceOnlyDomainInfos(kubernetesGatewayInfo.getVirtualGatewayId());
        ...
        for (String targetHost : targetHosts) {
            if (!dbResourceMap.containsKey(targetHost)){
                //创建域名
                DomainInfoDTO domainInfoDTO = transDomainInfo(targetHost, kubernetesGatewayInfo, gatewayDto);

...
public class DubboBindingServiceImpl implements IDubboBindingService {
    ...
    public long create(DubboBindingDto dubboBindingDto) {
        ...
        //publish to envoy
        if (ApiConst.ROUTE.equals(dubboBindingDto.getObjectType())) {
    ...
    public void processMethodWorks(DubboBindingDto dto) {
        ...
        RouteDto routeDto = routeService.get(dto.getObjectId());
        if (routeDto == null) {
            logger.error("未找到已发布路由信息，查询条件为 routeRuleProxyId:{}", dto.getObjectId());
            return;
        }
        //获取dubbo meta信息
        List<DubboMetaDto> dubboMetaDtos = dubboMetaService.findByCondition(routeDto.getVirtualGwId(), dto.getInterfaceName(), dto.getGroup(), dto.getVersion());
        //判断方法是否失效
    ...
    public ErrorCode checkAndComplete(DubboBindingDto dto) {
        ...
        //dubbo 路由应该只能支持单服务关联
        ServiceProxyDto serviceProxy = serviceProxyService.get(routeDto.getServiceIds().get(0));
        if (serviceProxy == null) {
            logger.info("参数校验失败，未找到已发布的服务，服务ID为 {}", routeDto.getServiceIds());
    ...
    private boolean publishToEnvoy(DubboBindingDto dto, boolean isDelete) {
        ...
        routeDto.setMetaMap(metaMap);
        return BaseConst.ERROR_RESULT != routeService.update(routeDto);
    }

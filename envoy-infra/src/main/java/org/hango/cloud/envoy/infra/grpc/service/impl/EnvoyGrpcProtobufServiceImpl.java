...
public class EnvoyGrpcProtobufServiceImpl implements IEnvoyGrpcProtobufService {
    ...
    @Override
    public EnvoyServiceProtobufProxy getServiceProtobufProxy(long serviceId) {
        Map<String, Object> params = Maps.newHashMap();
        params.put(BaseConst.SERVICE_ID, serviceId);
        ...
        if (serviceProtobufProxyList.size() > 1) {
            logger.warn("该服务在该网关下发布了多个pb文件，请手动删除，仅保留一个，serviceId为{}", serviceId);
        }
        return null;
    }
    ...
    public long saveServiceProtobufProxy(EnvoyServiceProtobufProxy serviceProtobufProxy) {
        EnvoyServiceProtobufProxy envoyServiceProtobufProxyInDB = getServiceProtobufProxy(serviceProtobufProxy.getServiceId());
    ...
    private ErrorCode publishServiceProtobuf(long serviceId, long virtualGatewayId, List<String> pbServiceList, String pbFileName, String pbFileContent, String protoDescriptorBin, List<String> publishServiceList) {
        if (!grpcProtobufRemoteClient.publishGrpcEnvoyFilterToAPIPlane(virtualGatewayId, protoDescriptorBin, publishServiceList)) {
    ...
    private ErrorCode processPbServicePublishStatus(long pbServiceId, long virtualGwId, boolean isPublish) {
        ...
        if (!grpcProtobufRemoteClient.publishGrpcEnvoyFilterToAPIPlane(virtualGwId, protoDescriptorBin, publishServiceList)) {
            return EnvoyErrorCode.PUBLISH_PROTOBUF_FAILED;
        }
    ...
    private void saveServiceProtobufProxy(long gwId, PbService pbService) {
        EnvoyServiceProtobuf serviceProtobufById = getServiceProtobufById(pbService.getPbId());
        EnvoyServiceProtobufProxy serviceProtobufProxy = getServiceProtobufProxy(serviceProtobufById.getServiceId());
    ...
    @Override
    public Pair<ErrorCode, PbCompileResultDto> checkPublishPbFile(long serviceId, MultipartFile file, List<String> pbServiceList) {
        ServiceProxyDto serviceInfoDb = serviceProxyService.get(serviceId);
        if (serviceInfoDb == null) {
            return new Pair<>(CommonErrorCode.NO_SUCH_SERVICE, null);
        }
        VirtualGatewayDto virtualGateway = virtualGatewayInfoService.get(serviceInfoDb.getVirtualGwId());
        if (virtualGateway == null) {
            return new Pair<>(CommonErrorCode.NO_SUCH_GATEWAY, null);
        }
        return envoyGrpcProtobufCompileService.checkPublishCompileResult(serviceId, serviceInfoDb.getVirtualGwId(), file, pbServiceList);
    }
    ...
    @Override
    public ErrorCode checkOfflinePbFile(long serviceId) {
        ServiceProxyDto serviceDto = serviceProxyService.get(serviceId);
        if (serviceDto == null) {
            return CommonErrorCode.NO_SUCH_SERVICE;
        }
        VirtualGatewayDto virtualGateway = virtualGatewayInfoService.get(serviceDto.getVirtualGwId());
        if (virtualGateway == null) {
            return CommonErrorCode.NO_SUCH_GATEWAY;
        }
        return CommonErrorCode.SUCCESS;
    ...
    @Override
    @Transactional
    public ErrorCode offlineServiceProtobuf(long serviceId) {
        //幂等
        EnvoyServiceProtobufProxy envoyServiceProtobufProxy = getServiceProtobufProxy(serviceId);
        if (envoyServiceProtobufProxy == null) {
            return CommonErrorCode.SUCCESS;
        }
        ErrorCode errorCode = offlineServiceProtobufToApiPlane(serviceId);
        if (!CommonErrorCode.SUCCESS.equals(errorCode)) {
            return errorCode;
        }
        deleteServiceProtobufProxy(serviceId);
        return CommonErrorCode.SUCCESS;
    }

    private ErrorCode offlineServiceProtobufToApiPlane(long serviceId) {
        ServiceProxyDto serviceDto = serviceProxyService.get(serviceId);
        VirtualGatewayDto virtualGatewayDto = virtualGatewayInfoService.get(serviceDto.getVirtualGwId());
        //获取当前已发布的服务
        List<EnvoyServiceProtobufProxy> envoyServiceProtobufProxyList = listServiceProtobufProxy(serviceDto.getVirtualGwId())
        ...
        if (CollectionUtils.isEmpty(envoyServiceProtobufProxyList)) {
            return grpcProtobufRemoteClient.deleteGrpcEnvoyFilterToAPIPlane(serviceDto.getVirtualGwId())? CommonErrorCode.SUCCESS : EnvoyErrorCode.OFFLINE_PROTOBUF_FAILED;
        }
        return doOfflineServiceProtobuf(serviceId, virtualGatewayDto.getId(), envoyServiceProtobufProxyList);
    }

    private ErrorCode doOfflineServiceProtobuf(long serviceId, Long vgId, List<EnvoyServiceProtobufProxy> envoyServiceProtobufProxyList) {
        List<String> sourcePathList = new ArrayList<>();
        List<File> pbFileList = new ArrayList<>();
        List<String> publishServiceList = new ArrayList<>();
        ...
        //更新
        boolean result = grpcProtobufRemoteClient.publishGrpcEnvoyFilterToAPIPlane(vgId, (String) resultMap.get(EnvoyGrpcProtobufCompileServiceImpl.DESC_FILE_BASE64), publishServiceList);
        if (!result) {
            return EnvoyErrorCode.PUBLISH_PROTOBUF_FAILED;
        }
        return CommonErrorCode.SUCCESS;
    ...
    private void deleteServiceProtobufProxy(long serviceId) {
        EnvoyServiceProtobufProxy serviceProtobufProxy = getServiceProtobufProxy(serviceId);
        if (serviceProtobufProxy != null) {

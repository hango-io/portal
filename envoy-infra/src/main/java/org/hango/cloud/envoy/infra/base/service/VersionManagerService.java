...
public class VersionManagerService {
    ...
    public ResourceDTO getResourceDTO(long vgId, long id, ResourceEnum resource){
        ...
        switch (resource) {
            case Route:
                RoutePO query = RoutePO.builder().virtualGwId(vgId).id(id).build();
                RoutePO proxyPO = routeMapper.selectOne(new QueryWrapper<>(query));
                if (proxyPO == null){
                    return null;
                }
                ...
                break;
            case Service:
                ServiceProxyDto serviceProxyInfo = serviceProxyService.get(id);

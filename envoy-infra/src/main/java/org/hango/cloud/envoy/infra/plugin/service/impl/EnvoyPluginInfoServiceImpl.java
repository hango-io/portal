...
public class EnvoyPluginInfoServiceImpl implements IEnvoyPluginInfoService {
    ...
    public boolean createPluginAndMakeRequest(BindingPluginDto bindingPluginDto, Operation operation, List<String> toBePublishedPluginList) {
        RouteRuleProxyDto routeRuleProxyInfo;
        // 路由使能状态，路由不使能则不发布（该标志不影响全局插件）
        String enableState = BaseConst.ROUTE_RULE_ENABLE_STATE;
        ...
        if (bindingPluginDto.isRoutePlugin()) {
            routeRuleProxyInfo = routeRuleProxyService
                    .getRouteRuleProxy(bindingPluginDto.getVirtualGwId(), bindingPluginDto.getBindingObjectId());
    ...
    private boolean requestForGatewayPlugin(GatewayPluginDto plugin, Map<String, Object> params) {
        ...
        VirtualGatewayDto virtualGateway = plugin.getVirtualGateway();
        if (virtualGateway == null || StringUtils.isEmpty(virtualGateway.getConfAddr())) {
            throw new IllegalArgumentException("VirtualGateway info is missing!");
        }
        ...
        return versionManagerService.publishPluginWithVersionManager(virtualGateway.getConfAddr() + PLANE_PORTAL_PATH, params, headers, plugin, resourceDTO);
    }
    ...
    private GatewayPluginDto createGlobalPlugin(BindingPluginDto bindingPluginInfo, List<String> pluginList) {
        // 路由信息不传
        List<String> hosts = domainInfoService.getEnableHosts(bindingPluginInfo.getBindingObjectId(), bindingPluginInfo.getVirtualGwId());
    ...
    private GatewayPluginDto createHostPlugin(BindingPluginDto bindingPluginInfo, List<String> pluginList) {
        // 路由信息不传
        DomainInfoPO domainInfoPO = domainInfoMapper.selectById(bindingPluginInfo.getBindingObjectId());
    ...
    private GatewayPluginDto createPlugin(BindingPluginDto bindingPluginInfo,
                                          RouteRuleProxyDto routeRuleProxyDto,
                                          String code,
                                          List<String> hosts,
                                          List<String> pluginList) {
        VirtualGatewayDto virtualGatewayDto = virtualGatewayInfoService.get(bindingPluginInfo.getVirtualGwId());
        ...
        gatewayPlugin.setVirtualGateway(virtualGatewayDto);
        ...
        if (routeRuleProxyDto != null) {
            gatewayPlugin.setRouteId(routeRuleProxyDto.getRouteRuleId());
            hosts = domainInfoService.getEnableHosts(routeRuleProxyDto.getProjectId(), routeRuleProxyDto.getVirtualGwId());

...
public class MultiClusterTask {
    ...
    public List<? extends VirtualGatewayDto> findVirtualGatewayInfo(){
        if (StringUtils.hasText(gwClusterNames)){
            ...
            return virtualGatewayInfoService.getGatewayInfoList(gwIds);
        }
    ...
    public Map<String, List<ResourceDTO>> describeResourceInfo(VirtualGatewayDto vgDto) {
        ...
        params.put("virtualGwId", vgId);
        List<ServiceProxyInfo> serviceProxyInfos = serviceProxyDao.getRecordsByField(params);
        List<ResourceDTO> serviceResources = serviceProxyInfos.stream().map(o -> convertService(o, gwClusterName, code)).collect(Collectors.toList());

        //查询路由
        RouteRuleQuery query = RouteRuleQuery.builder().virtualGwId(vgId).build();
        List<RouteRuleProxyPO> routeRuleProxyInfos = routeRuleProxyDao.getRouteRuleProxyList(query);
    ...
    public ResourceDTO convertRoute(RouteRuleProxyPO routeRuleProxyPO, String gwClusterName, String vgCode){
        String name = "qz" + "-" + routeRuleProxyPO.getRouteRuleId() + "-" + gwClusterName + "-" + vgCode;
        return ResourceDTO.of(routeRuleProxyPO.getVersion(), routeRuleProxyPO.getId(), name);
    }

    public ResourceDTO convertService(ServiceProxyInfo envoyServiceProxyInfo, String gwClusterName, String vgCode){
        String name = envoyServiceProxyInfo.getCode().toLowerCase(Locale.ROOT) + "-" + gwClusterName + "-" + vgCode;
        return ResourceDTO.of(envoyServiceProxyInfo.getVersion(), envoyServiceProxyInfo.getId(), name);
    }
